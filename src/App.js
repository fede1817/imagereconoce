...
function api(img_url, setBox, box) {
  ///////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  const PAT = "87c6261c8f594357af014e1052dab0f0";
...
//   )
//     .then((response) => response.json())
//     .then((result) => setColor(result.outputs[0].data.colors))
//     .catch((error) => console.log("error", error));
// }
function App() {
  ...
  useEffect(() => {
    if (input.length !== 0 && interruptor) {
      api(input, setBox, box);
    }
  });
  return (
    <div className="App">
      ...
      {login === "login" ? (
        <Singnin setLogin={log} />
      ) : login === "register" ? (
        <Register setLogin={log} />
      ) : (
        <div>
          <Navigation setLogin={log} />
          <Logo />
          <Rank />
          <ImageLinkForm
            setInput={setInput}
            ...
            setInterruptor={setInterruptor}
          />
          {interruptor && <FaceReconition input={input} box={box} />}
        </div>
      )}
    </div>
  );
